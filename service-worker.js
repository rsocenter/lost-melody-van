const CACHE_NAME = 'isaska-cache-v20-icon-b-gold-ui';
const urlsToCache = [
  "./",
  "index.html",
  "manifest.json",
...
self.addEventListener('install', event => { self.skipWaiting(); event.waitUntil(caches.open(CACHE_NAME).then(cache => cache.addAll(urlsToCache).catch(()=>{}))); });
